...
struct QGrid{T<:AbstractFloat, SIZE}
    k::T
    kF::T
    iscombined::Int
    grid::MVector{SIZE,T}
    ...
    head::T
    tail::T
    unilogs::SVector{5,Grid.UniLog{T}}

    function QGrid{T,SIZE}(k, kF, maxk, mink, MS) where {T<:AbstractFloat, SIZE}
        size = sum(MS)+1
        ...
        if abs(k-2kF)<2mink
            ...
            g0 = Grid.UniLog{T}([0.0, k0], 1, mink, M0, 1, true, [false, true])
            push!(unilogs,g0)
            for idx=g0.idx[1]:g0.idx[2]
                push!(grid, Grid._grid(g0, idx))
            end

            g1 = Grid.UniLog{T}([k0, k1], M0+1, mink, M1, 1, false, [false, true])
            push!(unilogs,g1)
            push!(unilogs,g1)
            for idx=g1.idx[1]:g1.idx[2]
                push!(grid, Grid._grid(g1, idx))
            ...
            g2 = Grid.UniLog{T}([k1, maxk], M0+M1+1, mink, M2, 1, true, [false, false])
            push!(unilogs,g2)
            push!(unilogs,g2)
            for idx=g2.idx[1]:g2.idx[2]
                push!(grid, Grid._grid(g2, idx))
        ...
        elseif abs(k)<2mink
            ...
            g0 = Grid.UniLog{T}([0.0, kF], 1, mink, MS[1]+MS[2]+MS[3]-1, 1, true, [false, true])
            push!(unilogs,g0)
            push!(unilogs,g0)
            push!(unilogs,g0)
            for idx=g0.idx[1]:g0.idx[2]
            ...
            g1 = Grid.UniLog{T}([kF, 2kF], MS[1]+MS[2]+MS[3]+1, mink, MS[4]-1, 1, false, [false, true])
            push!(unilogs,g1)
            for idx=g1.idx[1]:g1.idx[2]
                push!(grid, Grid._grid(g1, idx))
            end

            g2 = Grid.UniLog{T}([2kF, maxk], MS[1]+MS[2]+MS[3]+MS[4]+1, mink, MS[5]-1, 1, true, [false, false])
            push!(unilogs,g2)
            for idx=g2.idx[1]:g2.idx[2]
                push!(grid, Grid._grid(g2, idx))
            end
            ...
            k1, k2 = minimum([k,2kF]), maximum([k,2kF])
            bounds = [[0.0, k1/2],[k1/2, k1],[k1,(k1+k2)/2.0], [(k1+k2)/2.0, k2],[k2, maxk]]
            d2s = [true, false, true, false, true]
            isopen = [[false, true],[false, true],[false, true],[false, true],[false, false]]
            init = 1
...
"""
   function qGrid(k, kF, maxk, mink, MS)

Create a logarithmic q Grid, which is densest near momentum 0,  ``2k_F`` and a given momentum ``k``
...
- MS: a list of # of points in each segments, the total size will be sum(MS)+1
"""
@inline function qGrid(k, kF, maxk, mink, MS)
    return QGrid{Float64, sum(MS)+1}(k, kF, maxk, mink, MS)
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    qgrid = qGrid(0.001, 1.0, 10.0, 0.001, [4,4,4,4,4])
    println(qgrid.grid)
    qgrid = qGrid(0.999, 1.0, 10.0, 0.001, [4,4,4,4,4])
    println(qgrid.grid)
    qgrid = qGrid(2.0, 1.0, 10.0, 0.001, [4,4,4,4,4])
    println(qgrid.grid)
